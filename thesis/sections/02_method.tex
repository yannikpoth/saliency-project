...
To provide model-agnostic insights into choice behavior, two primary analyses were specified.

First, to directly test Hypothesis 1.2, a win-stay analysis was conducted. Win-stay probability—the conditional probability of selecting the same option on trial $t+1$ after it was rewarded on trial $t$—was calculated for each participant. These probabilities were computed separately for trials following non-salient wins versus salient wins. A paired-samples $t$-test is specified to compare these two conditions, with the Wilcoxon signed-rank test designated as a non-parametric alternative if the normality assumption is violated. This analysis directly assesses whether salient feedback increases the tendency to repeat a rewarded choice.

Second, as an exploratory step not specified in the pre-registration, a generalized linear mixed-effects model (GLMM) was fitted to provide a more nuanced, trial-by-trial description of learning. This approach moves beyond simple choice repetition to examine how feedback influences the \textit{quality} of decisions. A logistic GLMM was fitted to analyze factors influencing whether a choice was optimal (i.e., directed at the stimulus with the higher true reward probability). The dependent variable was a binary indicator of choice optimality on each trial. The fixed effects included the outcome of the previous trial (loss, non-salient win, or salient win) and the trial number (to control for time-on-task effects). A random intercept for each participant was included to account for baseline differences in choice strategy. This analysis helps to disentangle whether feedback promotes simple behavioral repetition or genuine learning of the task structure.
