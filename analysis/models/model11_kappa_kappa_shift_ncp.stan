/*      Reinforcement learning model: Model 11 - Kappa + Kappa Shift (NCP)
Last edit:  2025/11/27
Authors:    Poth, Yannik (YP)
            Geysen, Steven (SG)
Notes:      - Factorial design model 11/12: Kappa + Kappa shift (NCP Version)
...
Comments:   - Tests if salient feedback modulates perseveration (without learning rate shift)
  ...
  for (subi in 1:nSubs) {
    ...
    for (triali in 1:subTrials[subi]) {
      ...
      if ((choice[subi, triali] >= 1) && (reward[subi, triali] >= 0) && (salient_feedback[subi, triali] >= 0)) {
        ...
        // Calculate trial-specific alpha
