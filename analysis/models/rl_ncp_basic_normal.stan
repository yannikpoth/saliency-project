/*      Reinforcement learning model: Basic - NORMAL PRIORS / NCP VERSION
Last edit:  2025/06/19
Authors:    Poth, Yannik (YP)
Notes:      - This is the simplest baseline model. It includes only alpha and beta.
            - It will serve as the null model against which more complex models are compared.
            - Implements a Non-Centered Parameterization (NCP).
            - Uses weakly informative normal priors.
To do:      - Run as part of the full model comparison.
Comments:   - A robust, simple baseline is essential for any principled model comparison.
...
data {
  ...
  // Salient feedback is not used in this model, but included in data for consistency
  ...
  for (subi in 1:nSubs) {
    ...
    for (triali in 1:subTrials[subi]) {
      ...
      if ((choice[subi, triali] >= 1) && (reward[subi, triali] >= 0)) {
        ...
        vector[2] gq_choice_logits = current_beta_s_gq * qval_gq;
