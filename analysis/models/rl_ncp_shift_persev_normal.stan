/*      Reinforcement learning model: Alpha Shift + Perseveration - NORMAL PRIORS / NCP VERSION
Last edit:  2024/07/25
Authors:    Poth, Yannik (YP); Gemini
Notes:      - Based on rl_ncp_shift_perserv_uniform.stan
            - SUPERSEDES PREVIOUS VERSIONS due to convergence failure.
            - Implements a Non-Centered Parameterization (NCP).
            - Replaces unstable uniform priors with weakly informative normal priors to aid convergence.
                - Group-level means (alpha_mu_raw, beta_mu_raw): normal(0, 1.5)
                - Group-level SDs: normal(0, 1)
            - Includes alpha_shift parameter modulated by salient feedback.
            - Includes hierarchical perseveration/choice repetition parameter (kappa).
To do:      - This is the primary candidate for the final model. Must be validated for convergence.
Comments:   - This model represents the current best practice for this type of cognitive model,
              combining NCP with weakly informative priors to create a robust and stable model.
...
parameters {
  ...
  vector[nSubs] kappa_subj_z;         // Subject-specific perseveration strength offsets (z-scored)
}
...
transformed parameters {
  ...
  kappa_subj_raw = kappa_mu_raw + kappa_subj_z * kappa_sd_raw;
...
generated quantities {
  ...
  for (subi in 1:nSubs) {
    ...
    kappa[subi] = kappa_subj_raw[subi];             // Storing raw kappa in a clean variable
  ...
  for (subi in 1:nSubs) {
    ...
    real current_kappa_s_gq = kappa[subi]; // Uses subject's "clean" kappa from above
