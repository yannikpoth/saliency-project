/*      Reinforcement learning model: Model 12 - Full Model (NCP)
Last edit:  2025/11/27
Authors:    Poth, Yannik (YP)
            Geysen, Steven (SG)
Notes:      - Factorial design model 12/12: Full model (NCP Version)
...
parameters {
  ...
  vector[nSubs] kappa_shift_subj_raw_z;
}
...
transformed parameters {
  ...
  vector[nSubs] kappa_shift_subj_raw;
  ...
  kappa_shift_subj_raw = kappa_shift_mu_raw + kappa_shift_sd_raw * kappa_shift_subj_raw_z;
...
model {
  ...
  kappa_shift_sd_raw ~ uniform(0.0001, 10);
  ...
  kappa_shift_subj_raw_z ~ std_normal();
...
generated quantities {
  ...
  for (subi in 1:nSubs) {
    ...
    // Export both salient-condition parameter values and the salience effects.
  ...
  // Group-level salient parameter values and interpretable shifts.
