// basic_rl_model_NCP.stan - Non-Centered Parameterization Version
...
parameters {
  ...
  // Subject-level Raw Parameters (Standardized Deviations for Non-Centered Param.)
...
model {
  ...
  for (s in 1:nSubjects) {
    ...
    // Apply Non-Centered parameterization and Phi_approx transformation here
    real alpha_subj = Phi_approx(alpha_mu_raw + alpha_sd_raw * alpha_raw[s]);
    real beta_subj  = Phi_approx(beta_mu_raw  + beta_sd_raw * beta_raw[s]) * 3; 
...
generated quantities {
  ...
  for (s in 1:nSubjects) {
    // Calculate using the non-centered subject raw parameters
    alpha[s] = Phi_approx(alpha_mu_raw + alpha_sd_raw * alpha_raw[s]);
    beta[s]  = Phi_approx(beta_mu_raw  + beta_sd_raw * beta_raw[s]) * 3;
  }
  ...
  { // Local block for predictions
    ...
    for (s in 1:nSubjects) {
      ...
      real alpha_subj = alpha[s]; // Use already calculated interpretable alpha
      real beta_subj = beta[s];   // Use already calculated interpretable beta
