# Explanation of the `rl_cp_shift_normal.stan` Model

**Last Updated**: 2025/05/09

This document provides a conceptual overview of the hierarchical Bayesian reinforcement learning model implemented in `rl_cp_shift_normal.stan`. This version uses Normal priors for group means and Half-Normal priors for SDs. The model is designed to investigate how salient audiovisual feedback modulates learning rates in a restless two-armed bandit task. It estimates parameters such as base learning rate (`alpha`), a shift in learning rate due to salient cues (`alpha_shift`), and decision consistency (`beta`).
...
*   **Priors**:
    *   **Group-Level Means (`_mu_raw`)**: These are given weakly informative `normal(0, 1)` priors in this `_normal` version of the model.
    *   **Group-Level Standard Deviations (`_sd_raw`)**: These are given weakly informative `normal(0, 1) T[0, ]` priors (half-normal distributions, constrained to be positive) in this `_normal` version.
