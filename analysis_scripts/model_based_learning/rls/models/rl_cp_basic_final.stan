/*      Reinforcement learning model: Basic - FINAL COMBINED VERSION
Last edit:  2025/05/08
Authors:    Poth, Yannik (YP)
            Geysen, Steven (SG)
Notes:      - Uses normal priors for group-level raw parameters.
...
Comments:   - Intended as a robust baseline RL model.
...
model {
  ...
  alpha_mu_raw ~ normal(0, 1);   // Weakly informative prior
  beta_mu_raw  ~ normal(0, 1);   // Weakly informative prior
  
  // Group-level Standard Deviations (on the raw, unbounded scale, constrained positive)
  alpha_sd_raw ~ normal(0, 1) T[0, ]; // Half-normal (weakly informative)
  beta_sd_raw  ~ normal(0, 1) T[0, ]; // Half-normal (weakly informative)
