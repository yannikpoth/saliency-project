/*      Reinforcement learning model: Alpha Shift - NORMAL PRIORS VERSION
...
Notes:      - Based on rl_cp_basic_final.stan
            ...
            - This version uses Normal priors for group means (alpha_mu_raw, alpha_shift_mu_raw, beta_mu_raw)
              and Half-Normal priors for group-level SDs (alpha_sd_raw, alpha_shift_sd_raw, beta_sd_raw).
...
model {
  ...
  alpha_mu_raw ~ normal(0, 1);         // Weakly informative prior for base alpha
  alpha_shift_mu_raw ~ normal(0, 1);   // Weakly informative prior for alpha shift
  beta_mu_raw  ~ normal(0, 1);         // Weakly informative prior for beta
  
  // Group-level Standard Deviations (on the raw, unbounded scale, constrained positive)
  alpha_sd_raw ~ normal(0, 1) T[0, ];       // Half-normal for base alpha SD
  alpha_shift_sd_raw ~ normal(0, 1) T[0, ]; // Half-normal for alpha shift SD
  beta_sd_raw  ~ normal(0, 1) T[0, ];       // Half-normal for beta SD
