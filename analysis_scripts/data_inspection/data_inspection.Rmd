...
library(fs)
library(patchwork)
library(gt) # Added for enhanced table formatting
library(webshot2) # Added for saving gt tables as PNG
```
...
# 6.1 Pre-calculate Accuracy Metrics

```{r precalculate_accuracy_metrics}
# This chunk calculates accuracy metrics needed for the summary table later.
# It uses `all_task_data` which should be loaded in a previous chunk.

# Filter out missed trials (where choice is NA) and trials with fast RTs (e.g., < 150ms)
# Ensure this RT filter is consistent with how you define valid trials for accuracy.
accuracy_data_for_table <- all_task_data %>% 
  filter(!is.na(choice)) %>% 
  # Assuming reaction_time filtering is desired for accuracy metric in the table.
  # If not, this filter can be removed or adjusted.
...
accuracy_data_for_table <- accuracy_data_for_table %>%
  mutate(
  ...
  filter(!is.na(chose_better_option)) # Remove trials where accuracy couldn't be determined (e.g. NA reward_probs)

# Calculate mean accuracy per participant
accuracy_summary_for_table <- accuracy_data_for_table %>%
  group_by(subject_id) %>% 
  summarise(
    Accuracy = mean(chose_better_option, na.rm = TRUE),
    .groups = 'drop'
  )

cat("Accuracy summary for table pre-calculated.\n")
# print(accuracy_summary_for_table) # Optional: uncomment to see the raw accuracy data
```

# 6.2 Individual Participant Choice & Reward Diagnostics

```{r individual_choice_reward_diagnostics, fig.width=10, fig.height=6}
# Ensure a directory for these specific diagnostic plots exists
diag_figs_dir <- here::here("analysis_scripts", "figures", "individual_choice_diagnostics")
if (!dir.exists(diag_figs_dir)) {
  dir.create(diag_figs_dir, recursive = TRUE)
}

# Get unique subject IDs from the loaded data
unique_subject_ids <- sort(unique(all_task_data$subject_id)) # Sort for consistent column order
...
for (current_subject_id in unique_subject_ids) {
  ...
  # Minimal console output, diagnostics moved to table and plots
  # cat(paste("\n--- Processing Participant:", current_subject_id, "---\n")) 
  
  p_data <- all_task_data %>% 
    filter(subject_id == current_subject_id) %>%
    arrange(trial)

  p_data_valid <- p_data %>% 
    filter(!is.na(choice) & !is.na(reward))
  
  prop_choice_0 <- NA_real_
  prop_choice_1 <- NA_real_
  overall_reward_rate_p <- NA_real_
  win_stay_prob_p <- NA_real_
  current_accuracy <- NA_real_ # Initialize accuracy for the current subject

  if (nrow(p_data_valid) > 0) {
    prop_choice_0 <- sum(p_data_valid$choice == 0) / nrow(p_data_valid)
    prop_choice_1 <- 1 - prop_choice_0
    overall_reward_rate_p <- sum(p_data_valid$reward == 1) / nrow(p_data_valid)
    
    win_stay_count_p <- 0
    opportunities_to_win_stay_p <- 0
    if (nrow(p_data_valid) > 1) {
      for (i in 1:(nrow(p_data_valid) - 1)) {
        if (p_data_valid$reward[i] == 1) {
          opportunities_to_win_stay_p <- opportunities_to_win_stay_p + 1
          if (p_data_valid$choice[i+1] == p_data_valid$choice[i]) {
    ...
    # Fetch pre-calculated accuracy for this participant
    # Ensure accuracy_summary_for_table is available from the previous chunk
    if (exists("accuracy_summary_for_table") && current_subject_id %in% accuracy_summary_for_table$subject_id) {
      current_accuracy <- accuracy_summary_for_table$Accuracy[accuracy_summary_for_table$subject_id == current_subject_id]
    }
    ...
    p_behavior_plot <- ggplot(p_plot_df, aes(x = trial)) +
      ...
      geom_point(aes(y = choice + ifelse(reward == 1, 0.2, -0.2), shape = plot_reward), size = 3) +
      scale_shape_manual(values = c("No Reward" = 4, "Reward" = 16), name = "Reward") +
      scale_y_continuous(breaks = c(0, 1), labels = c("Stimulus 0", "Stimulus 1"), limits = c(-0.5, 1.5)) +
    ...
    # Suppress printing plot to console if many subjects, to keep Rmd output cleaner
    # It will still be saved.
    # print(p_behavior_plot) 
    
    plot_filename <- file.path(diag_figs_dir, paste0("participant_", current_subject_id, "_choice_reward_plot.png"))
    ggsave(filename = plot_filename, plot = p_behavior_plot, width = 10, height = 4, bg = "white", dpi = 150)
    # cat(paste("Plot for participant", current_subject_id, "saved to:", plot_filename, "\n"))
  }
  
  all_participant_stats[[current_subject_id]] <- c(
    `Prop. S0` = prop_choice_0,
    `Prop. S1` = prop_choice_1,
    `Rew. Rate` = overall_reward_rate_p,
    `Accuracy` = current_accuracy, # Added Accuracy
...
if (length(all_participant_stats) > 0) {
  ...
  cat("\n\n--- Overall Participant Behavioral Summary Table ---\n")
  
  summary_table_gt <- summary_df %>% 
    gt(rowname_col = "Metric") %>% 
    tab_header(
      title = "Participant Behavioral Summary",
      subtitle = "Key metrics derived from task choices and rewards. Accuracy based on choosing higher reward probability option."
    ) %>%
    fmt_number(
      columns = everything(),
      decimals = 2,
    ...
    data_color(
      columns = where(is.numeric),
      rows = `Metric` == "Accuracy", # Added styling for Accuracy
      colors = scales::col_numeric(
        palette = "Blues", # Or "Oranges", "Purples" etc.
    ...
    tab_source_note(
      source_note = md("**Note**: `Prop. S0` refers to the proportion of trials where Stimulus 0 (original coding) was chosen. Accuracy calculated against choosing the option with higher `reward_prob`.")
    )
    
  print(summary_table_gt)
  
  gt_html_filename <- file.path(diag_figs_dir, "participant_behavioral_summary_table.html")
  gtsave(summary_table_gt, filename = gt_html_filename)
  cat(paste("\nSummary table (HTML) saved to:", gt_html_filename, "\n"))

  gt_png_filename <- file.path(diag_figs_dir, "participant_behavioral_summary_table.png")
  gtsave(summary_table_gt, filename = gt_png_filename, vwidth = 1200, vheight = 800) # Adjust vwidth/vheight as needed
  cat(paste("Summary table (PNG) saved to:", gt_png_filename, "\n"))
  
} else {
  cat("No participant statistics collected to generate summary table.\n")
}
...
# 7.1 Individual Participant Accuracy

```{r individual_accuracy_analysis, fig.width=7, fig.height=max(4, 0.3 * n_subjects_accuracy)}
# --- Display Sorted Accuracy Table ---
# Sort accuracy_summary by mean_accuracy for easy inspection
sorted_accuracy_summary <- accuracy_summary %>%
  arrange(mean_accuracy) %>%
  mutate(below_chance = mean_accuracy < 0.5)

print("Individual Participant Accuracies (sorted):")
print(knitr::kable(sorted_accuracy_summary, digits = 3, caption = "Individual participant mean accuracies, sorted. 'below_chance' is TRUE if accuracy < 0.5."))

# --- Visualize Individual Accuracies with a Dot Plot ---

# Ensure n_subjects_accuracy is available (calculated in the previous chunk)
if (!exists("n_subjects_accuracy")) {
  n_subjects_accuracy <- nrow(accuracy_summary)
}

# Create the dot plot
individual_accuracy_plot <- ggplot(sorted_accuracy_summary, 
                                   aes(x = mean_accuracy, 
  ...
  geom_text(aes(label = sprintf("%.3f", mean_accuracy),
                hjust = ifelse(mean_accuracy < 0.5, 1.3, -0.3)), # Adjusted hjust for more space
            vjust = 0.5,
            size = 2.5,
            check_overlap = TRUE) + # Avoid overlapping text if points are too close
  scale_color_manual(values = c("TRUE" = "firebrick", "FALSE" = "black"),
                     name = "Below Chance (0.5)",
                     labels = c("FALSE" = "No", "TRUE" = "Yes")) +
  scale_x_continuous(limits = c(min(0, min(sorted_accuracy_summary$mean_accuracy, na.rm = TRUE) - 0.05), 
                                 max(1, max(sorted_accuracy_summary$mean_accuracy, na.rm = TRUE) + 0.05)),
                     breaks = seq(0, 1, by = 0.1)) +
  labs(
    title = "Individual Participant Accuracy",
    subtitle = paste0("Each point represents a participant (N=", n_subjects_accuracy, "). Red line at chance level (0.5)."),
    x = "Mean Accuracy",
    y = "Participant ID (Sorted by Accuracy)"
  ) +
  ...
  theme(
    ...
    legend.position = "top",
    axis.text.y = element_text(size = rel(0.8)) # Adjust y-axis text if many subjects
  )

# Save the individual accuracy plot
ggsave(
  filename = file.path(figs_dir_main, "individual_accuracy_dotplot.png"),
  plot = individual_accuracy_plot,
  width = 7,
  height = max(4, 0.25 * n_subjects_accuracy), # Dynamic height based on number of subjects
  units = "in",
  dpi = 300,
  limitsize = FALSE # Allow plot to exceed default size limits if many subjects
)

# Display the plot
individual_accuracy_plot

```

# 7.2 Individual Participant Accuracy (Sorted by Participant ID)

```{r individual_accuracy_by_id_plot, fig.width=7, fig.height=max(4, 0.3 * n_subjects_accuracy)}
# --- Prepare data for this plot ---
# Ensure subject_id is character for proper sorting and `below_chance` is present.
# Data will be sorted for plotting by participant ID.
plot_data_by_id <- accuracy_summary %>%
  mutate(below_chance = mean_accuracy < 0.5)

# --- Visualize Individual Accuracies, Sorted by Participant ID ---

# Ensure n_subjects_accuracy is available (calculated in a previous chunk)
if (!exists("n_subjects_accuracy")) {
  n_subjects_accuracy <- nrow(accuracy_summary)
}

# Create the dot plot, sorted by participant ID
# Participant '01' (or lowest ID) will be at the top, descending.
individual_accuracy_by_id_plot <- ggplot(plot_data_by_id,
                                   aes(x = mean_accuracy,
                                       y = factor(subject_id, levels = rev(sort(unique(plot_data_by_id$subject_id)))),
                                       color = below_chance)) +
  geom_point(size = 2.5, alpha = 0.8) +
  geom_vline(xintercept = 0.5, linetype = "dashed", color = "red", size = 0.7) +
  geom_text(aes(label = sprintf("%.3f", mean_accuracy),
                hjust = ifelse(mean_accuracy < 0.5, 1.3, -0.3)), # Adjusted hjust
            vjust = 0.5,
            size = 2.5,
            check_overlap = TRUE) +
  scale_color_manual(values = c("TRUE" = "firebrick", "FALSE" = "black"),
                     name = "Below Chance (0.5)",
                     labels = c("FALSE" = "No", "TRUE" = "Yes")) +
  scale_x_continuous(limits = c(min(0, min(plot_data_by_id$mean_accuracy, na.rm = TRUE) - 0.05),
                                 max(1, max(plot_data_by_id$mean_accuracy, na.rm = TRUE) + 0.05)),
                     breaks = seq(0, 1, by = 0.1)) +
  labs(
...
# Ensure the main figures directory exists (defined in previous chunks)
# figs_dir_main <- here::here("analysis_scripts", "figures") 

# Save the new plot with a different filename
ggsave(
  filename = file.path(figs_dir_main, "individual_accuracy_by_id_dotplot.png"),
  plot = individual_accuracy_by_id_plot,
  width = 7,
  height = max(4, 0.25 * n_subjects_accuracy), # Dynamic height
  units = "in",
  dpi = 300,
  limitsize = FALSE
)
...
# Display the plot
individual_accuracy_by_id_plot
